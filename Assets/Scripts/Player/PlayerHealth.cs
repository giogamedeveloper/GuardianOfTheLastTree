    ...
    IDamageable<float>
{
        ...
        Vector3 impactPosition = default(Vector3))
    {
        ...
        GameManager.Instance.GetCurrentHealth(currentHealth);
    ...
    {
        float previousMaxHealth = maxHealth; // Guardar el maxHealth anterior
        maxHealth = value; // Actualizar vida máxima

        _previewHealth = currentHealth / previousMaxHealth; // Normalizar vida actual con el viejo max
        currentHealth += 10; // Curar 10 puntos
        currentHealth = Mathf.Clamp(currentHealth, 0, maxHealth); // Asegurar que no exceda el máximo
